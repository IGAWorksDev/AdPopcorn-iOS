Pod::Spec.new do |s|
s.name         = "AdPopcorn"
s.version      = "1.0.0"
s.summary      = "AdPopcorn.framework"
s.homepage     = "https://github.com/IGAWorksDev/AdPopcorn-iOS"

s.license      = {
:type => 'Commercial',
:text => <<-LICENSE
All text and design is copyright 2006-2016 igaworks, Inc.

All rights reserved.

https://github.com/IGAWorksDev/AdPopcorn-iOS
LICENSE
}

s.platform = :ios, '6.0'
s.author       = { "wonje,song" => "ricky@igaworks.com" }
s.source       = { :git => "https://github.com/IGAWorksDev/AdPopcorn-iOS.git", :tag => "#{s.version}" }
s.resources = "AdPopcorn.bundle"
s.ios.vendored_frameworks = 'AdPopcorn.framework'
s.frameworks = 'ImageIO', 'MessageUI', 'SafariServices', 'MediaPlayer', 'AdSupport', 'AVKit'
s.libraries = 'xml2'
s.xcconfig     = { 'HEADER_SEARCH_PATHS' => '"$(SDKROOT)/usr/include/libxml2"' }
end
